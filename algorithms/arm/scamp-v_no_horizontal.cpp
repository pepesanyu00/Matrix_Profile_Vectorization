...
#pragma omp for schedule(dynamic)
    for (ITYPE ri = 0; ri < Ndiags; ri++) {
      ...
      vlOuter = svwhilelt_b64((ITYPE)0,tam_vl); // Predicate for outer loop operations
      ...
      for (ITYPE j = diag + 1; j < profileLength; j++) {
        ...
        vlInner = svwhilelt_b64((ITYPE)0,tam_vl); // Predicate for inner loop operations
        ...
        DTYPE correlation_inner[tam_vl];
